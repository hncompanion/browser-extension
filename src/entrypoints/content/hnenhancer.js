...
class HNEnhancer {
    ...
    async summarizeAllComments(refreshCache = false) {
        ...
        if(!refreshCache && await this.canUseCachedSummaries()) {
            this.summaryPanel.updateContent({
                title: 'Post Summary',
    ...
    async showSummaryInPanel(summary, fromCache, duration, commentPathToIdMap = null) {
        ...
        const subtitle = fromCache
            ? `Summary generated by <strong>HNCompanion</strong> and cached <strong>${duration || 'some time'}</strong> ago.` +
                `<br/>Click <a href="#" id="refresh-summary-cache-link">here</a> to refresh the cache.`
            : (aiProvider
        ...
        const refreshCacheLink = this.summaryPanel.panel.querySelector('#refresh-summary-cache-link');
        if (refreshCacheLink) {
            refreshCacheLink.addEventListener('click', (e) => {
                e.preventDefault();
