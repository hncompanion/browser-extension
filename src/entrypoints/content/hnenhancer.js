...
class HNEnhancer {
    ...
    async summarizeAllComments() {
        ...
        if(cachedSummary && cachedSummary.length > 0) {
            ...
            await this.showSummaryInPanel(cachedSummary, true, 0);
            return;
        }
...
---`;
    ...
    async showSummaryInPanel(summary, fromCache, duration, commentPathToIdMap = null) {
        ...
        const subtitle = fromCache
            ? `Summary generated by <strong>HNCompanion</strong> and cached <strong>${duration || '{{x}}'} min</strong> ago.`
            : (aiProvider
                ? `Summarized using <strong>${aiProvider} ${model || 'LLM'}</strong> in <strong>${duration || '0'} secs</strong>.`
                : '');
    ...
    resolveCommentBacklinks(text, commentPathToIdMap) {
        ...
        // // First replace paths in format [1], [1.1] etc. Replace each match with an HTML link
        // let modifiedText = text.replace(pathRegex, (match, path) => {
        //     const id = commentPathToIdMap.get(path);
        //     if (!id) {
        //         return match; // If no ID found, return original text
        //     }
        ...
        // });
        //
        // console.log('after replacmodifiedText);
        //
        // // Then replace direct HN URLs with comment IDs in the hash portion
        // // This regex captures both raw URLs and URLs inside href attributes
        // modifiedText = modifiedText.replace(urlRegex, (match, hrefStart, commentId, hrefEnd) => {
        //     // If this URL is inside an href attribute (hrefStart exists),
        //     // just modify the href to use # and add data attributes
        //     if (hrefStart) {
        //         return `href="#" title="Go to comment #${commentId}" data-comment-link="true" data-comment-id="${commentId}"`;
        //     }
        //     // Otherwise it's a raw URL, replace with full anchor tag
        //     return ` <a href="#"
        //            title="Go to comment #${commentId}"
        //            data-comment-link="true" data-comment-id="${commentId}"
        //            style="color: rgb(130, 130, 130); text-decoration: underline;"
