...
---`;
    ...
    async showSummaryInPanel(summary, commentPathToIdMap, duration, cached = false) {
        ...
        formattedSummary = this.replacePathsWithCommentLinks(formattedSummary, commentPathToIdMap);

        const {aiProvider, model} = await this.getAIProviderModel();
        const metadata = cached ? `Summary cached by <strong>HNCompanion</strong> ${duration ?? 'x'} min ago` :
